...
func _ready() -> void:
	...
	original_star_y = star.position.y
...
func _process(dt: float) -> void:
	...
	trailing_star.position = star.position
...
func main_state(dt: float):
	...
	star.position.y = lerp(star.position.y, original_star_y, 15.0 * dt)
...
func settings_state(dt: float):
	star.position.y = lerp(star.position.y, original_star_y - 36, 15.0 * dt)
